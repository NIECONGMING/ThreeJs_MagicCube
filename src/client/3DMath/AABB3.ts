...
// Query for dimentions
    ...
    public corner(i:number):Vector3
     {
    ...
    public add(  p:Vector3) {
        ...
        if (p.y < this.min.x) this.min.y = p.y;
        if (p.y > this.max.x) this.max.y = p.y;
        if (p.z < this.min.x) this.min.z = p.z;
        if (p.z > this.max.x) this.max.z = p.z;
    }
    ...
    public _add(box:AABB3) {
    
        // Expand the box as necessary.
    
        if (box.min.x < this.min.x) this.min.x = box.min.x;
        if (box.min.x > this.max.x) this.max.x = box.min.x;
        if (box.min.y < this.min.x) this.min.y = box.min.y;
        if (box.min.y > this.max.x) this.max.y = box.min.y;
        if (box.min.z < this.min.x) this.min.z = box.min.z;
        if (box.min.z > this.max.x) this.max.z = box.min.z;
    }
    ...
    public setToTransformedBox(box:AABB3, m:Matrix4) {
    ...
    public rayIntersect( rayOrg: Vector3 ,rayDelta:Vector3,	returnNormal:Vector3) 
    {
    
        // We'll return this huge number if no intersection
    
        let kNoIntersection:number = 1e30;
        ...
        switch (which) {
            ...
            case 1: // intersect with xz plane
            {
                ...
                if (x < this.min.x || x > this.max.x) return kNoIntersection;
                let z = rayOrg.z + rayDelta.z*t;
                if (z < this.min.z || z > this.max.z) return kNoIntersection;
    ...
    public classifyPlane(n:Vector3,d:number)  {
        ...
        if (n.x > 0.0) {
            minD = n.x*this.min.x; maxD = n.x*this.max.x;
        } else {
            minD = n.x*this.max.x; maxD = n.x*this.min.x;
        }
    
        if (n.y > 0.0) {
            minD += n.y*this.min.y; maxD += n.y*this.max.y;
        } else 
        {
            minD += n.y*this.max.y; maxD += n.y*this.min.y;
        }
    
        if (n.z > 0.0){
            minD += n.z*this.min.z; maxD += n.z*this.max.z;
        } else {
            minD += n.z*this.max.z; maxD += n.z*this.min.z;
        }
